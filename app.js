...
function FireworksCanvas({ phase, word }) {
  ...
  useEffect(() => {
    ...
    const createWordTargets = (text) => {
      if (!text) return [];
      const fontSize = Math.max(64, Math.min(width * 0.14, 160));
    ...
    const startWordSequence = (text) => {
      ...
      const burstY = height * (0.24 + Math.random() * 0.12);
      sequence = {
        stage: 'falling',
        elapsed: 0,
        burstX,
    ...
    const updateSpecialSequence = (delta) => {
      ...
      if (sequence.stage === 'falling') {
        ...
        for (const particle of wordParticles) {
          ...
          particle.vx += dx * 0.0038 * (delta / 16.67);
          particle.vy += dy * 0.0038 * (delta / 16.67);
          particle.vx *= 0.89;
          particle.vy *= 0.89;
          particle.x += particle.vx * (delta / 16.67);
          particle.y += particle.vy * (delta / 16.67);
        ...
        if (settled > wordParticles.length * 0.74 || sequence.elapsed > 950) {
    ...
    const drawSpecialSequence = () => {
      ...
      if (sequence.stage === 'falling') {
        ...
        drawGlow(bomb.x, bomb.y + 8, 20, '#ff4961', 0.12);

        context.translate(bomb.x, bomb.y);
        context.rotate(bomb.rotation);
        context.fillStyle = '#f5f0ea';
        context.beginPath();
        context.ellipse(0, 0, 8.2, 13, 0, 0, Math.PI * 2);
        context.fill();
        context.fillStyle = '#ff4961';
        context.beginPath();
        context.arc(0, 12, 2.1, 0, Math.PI * 2);
        context.fill();
        context.restore();
      }

      if (sequence.stage === 'burst') {
        drawGlow(sequence.burstX, sequence.burstY, 64, '#ff4961', 0.1);
      }
      ...
      if (wordParticles.length) {
        for (const particle of wordParticles) {
          ...
          context.fill();
          context.restore();
          drawGlow(particle.x, particle.y, 9, particle.color, Math.max(0, (particle.alpha ?? 1) * 0.12));
        }
      }
    };
